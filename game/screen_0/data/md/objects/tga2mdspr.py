...
# TGA to MD sprites
#
# python tga2mdspr.py INPUT_IMAGE.tga OUT_FOLDER
#======================================================================
...
if len(sys.argv) != 1+4:
 print("tga2mdspr usage: input_tga out_folder spr_width spr_height")
 exit()
...
if has_pal == True:
 output_file = open(user_folder+user_thisname+"_pal.bin","wb")
 d = pal_len
 while d:
  d -= 1
...
if has_img == True:
 art_file = open(user_folder+user_thisname+"_art.bin","wb")
 map_file = open(user_folder+user_thisname+"_map.bin","wb")
 numof_frames = int(img_height/FRAME_HGHT)

 if PLC_MODE == True:
  plc_file = open(user_folder+user_thisname+"_plc.bin","wb")
 ...
 for curr_frame in range(0,numof_frames):
  ...
  x_cell = 0
  x_pos = (FRAME_WDTH>>1)*-1
  x_read = x_csize
  while x_read:
   y_cell = 0
   y_pos  = (FRAME_HGHT>>1)*-1
   y_read = y_csize
   while y_read:
